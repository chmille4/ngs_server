...
bool vcf_file::read_index_file(const string &index_filename)
{
	...
	{
		warning("VCF index is older than VCF file. Will regenerate.");
		return false;
	}

	printLOG("Reading Index file.\n");
	...
	{	// Doesn't appear to be an index file
		gzclose(in);
		warning("Index file doesn't appear to be valid. Will (try to) overwrite.\n");
		return false;
	}
...
{
	printLOG("Writing Index file.\n");
	big_endian_machine = is_big_endian();

	gzFile out = gzopen(index_filename.c_str(), "wb");
	if (out == NULL)
	{
		warning("Could not write index file.\n");
		return;
	}
